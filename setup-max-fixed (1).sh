...
_tg_send_file() {
    ...
    [[ -z "${BOT_TOKEN:-}" || -z "${CHAT_ID:-}" || ! -f "$filepath" ]] && return
...
show_box_ssh() {
    ...
    printf  "  ${A1}│${NC} 🔌 ${DIM}SSH Port   ${NC}: ${Y}22, 990, 446, 110, 444${NC}\n"
    printf  "  ${A1}│${NC} 🔌 ${DIM}SSH Non-TLS${NC}: ${Y}80, 2086${NC}\n"
    printf  "  ${A1}│${NC} 🔒 ${DIM}SSH TLS    ${NC}: ${Y}443, 2087${NC}\n"
    printf  "  ${A1}│${NC} 🔌 ${DIM}Dropbear   ${NC}: ${Y}109, 143${NC}\n"
    printf  "  ${A1}│${NC} 🔒 ${DIM}SSL/Stunnel${NC}: ${Y}445, 777${NC}\n"
    printf  "  ${A1}│${NC} 🔌 ${DIM}WS HTTP    ${NC}: ${Y}80 (/ws-ssh)${NC}\n"
    printf  "  ${A1}│${NC} 🔒 ${DIM}WS HTTPS   ${NC}: ${Y}443 (/ws-ssh)${NC}\n"
    printf  "  ${A1}│${NC} 📡 ${DIM}Proxy Port ${NC}: ${Y}0${NC}\n"
...
show_box_xray() {
    ...
    case "$proto" in
        VMess)
            printf  "  ${A1}│${NC} 🔌 ${DIM}Port w/o TLS${NC}: ${Y}80, 8080, 2086, 8880${NC}\n"
            printf  "  ${A1}│${NC} 🔒 ${DIM}Port TLS    ${NC}: ${Y}443, 2083, 2087, 8443${NC}\n"
            printf  "  ${A1}│${NC} 🛣  ${DIM}WS Path     ${NC}: ${Y}/vmess${NC}\n"
            printf  "  ${A1}│${NC} 📡 ${DIM}Protokol    ${NC}: ${Y}VMess, h2, WS, gRPC, TLS${NC}\n" ;;
        VLess)
            printf  "  ${A1}│${NC} 🔌 ${DIM}Port w/o TLS${NC}: ${Y}80, 8080, 2086, 8880${NC}\n"
            printf  "  ${A1}│${NC} 🔒 ${DIM}Port TLS    ${NC}: ${Y}443, 2083, 2087, 8443${NC}\n"
            ...
            printf  "  ${A1}│${NC} 📡 ${DIM}Protokol    ${NC}: ${Y}VLess, h2, WS, gRPC, TLS${NC}\n" ;;
        Trojan)
            printf  "  ${A1}│${NC} 🔒 ${DIM}Port TLS    ${NC}: ${Y}443, 2083, 2087, 8443${NC}\n"
            printf  "  ${A1}│${NC} 🔌 ${DIM}gRPC TLS    ${NC}: ${Y}8443${NC}\n"
            printf  "  ${A1}│${NC} 🛣  ${DIM}WS Path     ${NC}: ${Y}/trojan-ws${NC}\n"
            printf  "  ${A1}│${NC} 🛣  ${DIM}gRPC SVC    ${NC}: ${Y}trojan-grpc${NC}\n"
            printf  "  ${A1}│${NC} 📡 ${DIM}Protokol    ${NC}: ${Y}Trojan, h2, WS, gRPC, TLS${NC}\n" ;;
    esac
    echo -e "  ${A1}├─────────────────────────────────────────────────────────${NC}"
    printf  "  ${A1}│${NC} 🔒 ${DIM}MaxLogin${NC} : ${Y}%s device${NC}\n" "$maxl"
    printf  "  ${A1}│${NC} 📅 ${DIM}Expired ${NC} : ${Y}%s${NC}\n" "$exp"
...
# ════════════════════════════════════════════════════════════
install_deps() {
    inf "Memeriksa & memperbaiki DNS..."
    if ! ping -c 1 -W 2 8.8.8.8 &>/dev/null; then
        echo "nameserver 8.8.8.8" > /etc/resolv.conf
        echo "nameserver 1.1.1.1" >> /etc/resolv.conf
        ok "DNS diperbaiki ke 8.8.8.8 / 1.1.1.1"
    fi
...
ssh_add() {
    ...
    _tg_send "✅ <b>Akun SSH Baru — MAX PANEL</b>
━━━━━━━━━━━━━━━━━━━━━━
...
🔒 SSL/Stun. : <code>445, 777</code>
🔌 WS HTTP   : <code>80 (/ws-ssh)</code>
🔒 WS HTTPS  : <code>443 (/ws-ssh)</code>
━━━━━━━━━━━━━━━━━━━━━━
🔒 MaxLogin  : <code>${ml} device</code>
...
vmess_add() {
    ...
    # Generate semua vmess link variant
    local vmess_tls2083 vmess_tls2087 vmess_tls8443 vmess_ntls8080 vmess_ntls2086 vmess_ntls8880
    vmess_tls2083=$(printf '%s' "{\"v\":\"2\",\"ps\":\"${u}-TLS-2083\",\"add\":\"${dom}\",\"port\":\"2083\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${dom}\",\"path\":\"/vmess\",\"tls\":\"tls\",\"sni\":\"${dom}\"}" | base64 -w0)
    vmess_tls2087=$(printf '%s' "{\"v\":\"2\",\"ps\":\"${u}-TLS-2087\",\"add\":\"${dom}\",\"port\":\"2087\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${dom}\",\"path\":\"/vmess\",\"tls\":\"tls\",\"sni\":\"${dom}\"}" | base64 -w0)
    vmess_tls8443=$(printf '%s' "{\"v\":\"2\",\"ps\":\"${u}-TLS-8443\",\"add\":\"${dom}\",\"port\":\"8443\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${dom}\",\"path\":\"/vmess\",\"tls\":\"tls\",\"sni\":\"${dom}\"}" | base64 -w0)
    vmess_ntls8080=$(printf '%s' "{\"v\":\"2\",\"ps\":\"${u}-HTTP-8080\",\"add\":\"${dom}\",\"port\":\"8080\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${dom}\",\"path\":\"/vmess\",\"tls\":\"none\"}" | base64 -w0)
    vmess_ntls2086=$(printf '%s' "{\"v\":\"2\",\"ps\":\"${u}-HTTP-2086\",\"add\":\"${dom}\",\"port\":\"2086\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${dom}\",\"path\":\"/vmess\",\"tls\":\"none\"}" | base64 -w0)
    vmess_ntls8880=$(printf '%s' "{\"v\":\"2\",\"ps\":\"${u}-HTTP-8880\",\"add\":\"${dom}\",\"port\":\"8880\",\"id\":\"${uuid}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${dom}\",\"path\":\"/vmess\",\"tls\":\"none\"}" | base64 -w0)

    local ip dom; ip=$(get_ip); dom=$(get_domain)
    _tg_send "✅ <b>Akun VMess Baru — MAX PANEL</b>
━━━━━━━━━━━━━━━━━━━━━━
👤 Remark : <code>${u}</code>
🔑 UUID   : <code>${uuid}</code>
🌐 Host   : <code>${dom}</code>
...
📅 Expired  : <code>${exp}</code>
━━━━━━━━━━━━━━━━━━━━━━
🔗 <b>Link TLS (Port 443):</b>
<code>vmess://${vmess_tls}</code>

🔗 <b>Link TLS (Port 2083):</b>
<code>vmess://${vmess_tls2083}</code>

🔗 <b>Link TLS (Port 2087):</b>
<code>vmess://${vmess_tls2087}</code>

🔗 <b>Link TLS (Port 8443):</b>
<code>vmess://${vmess_tls8443}</code>

🔗 <b>Link HTTP (Port 80):</b>
<code>vmess://${vmess_ntls}</code>

🔗 <b>Link HTTP (Port 8080):</b>
<code>vmess://${vmess_ntls8080}</code>

🔗 <b>Link HTTP (Port 2086):</b>
<code>vmess://${vmess_ntls2086}</code>

🔗 <b>Link HTTP (Port 8880):</b>
<code>vmess://${vmess_ntls8880}</code>"
...
vless_add() {
    ...
    echo ""
    local ip dom; ip=$(get_ip); dom=$(get_domain)
    _tg_send "✅ <b>Akun VLess Baru — MAX PANEL</b>
━━━━━━━━━━━━━━━━━━━━━━
👤 Remark : <code>${u}</code>
🔑 UUID   : <code>${uuid}</code>
🌐 Host   : <code>${dom}</code>
...
━━━━━━━━━━━━━━━━━━━━━━
🔗 <b>WS TLS (443):</b>
<code>vless://${uuid}@${dom}:443?path=/vless&security=tls&encryption=none&host=${dom}&type=ws&sni=${dom}#${u}-TLS</code>

🔗 <b>WS TLS (2083):</b>
<code>vless://${uuid}@${dom}:2083?path=/vless&security=tls&encryption=none&host=${dom}&type=ws&sni=${dom}#${u}-TLS-2083</code>

🔗 <b>WS TLS (2087):</b>
<code>vless://${uuid}@${dom}:2087?path=/vless&security=tls&encryption=none&host=${dom}&type=ws&sni=${dom}#${u}-TLS-2087</code>

🔗 <b>WS TLS (8443):</b>
<code>vless://${uuid}@${dom}:8443?path=/vless&security=tls&encryption=none&host=${dom}&type=ws&sni=${dom}#${u}-TLS-8443</code>

🔗 <b>WS HTTP (80):</b>
<code>vless://${uuid}@${dom}:80?path=/vless&encryption=none&host=${dom}&type=ws#${u}-HTTP</code>

🔗 <b>WS HTTP (8080):</b>
<code>vless://${uuid}@${dom}:8080?path=/vless&encryption=none&host=${dom}&type=ws#${u}-HTTP-8080</code>

🔗 <b>WS HTTP (2086):</b>
<code>vless://${uuid}@${dom}:2086?path=/vless&encryption=none&host=${dom}&type=ws#${u}-HTTP-2086</code>

🔗 <b>WS HTTP (8880):</b>
<code>vless://${uuid}@${dom}:8880?path=/vless&encryption=none&host=${dom}&type=ws#${u}-HTTP-8880</code>

🔗 <b>gRPC TLS (443):</b>
<code>vless://${uuid}@${dom}:443?mode=gun&security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=${dom}#${u}-gRPC</code>

🔗 <b>gRPC TLS (8443):</b>
<code>vless://${uuid}@${dom}:8443?mode=gun&security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=${dom}#${u}-gRPC-ALT</code>"
    pause
}
...
trojan_add() {
    ...
    echo ""
    local ip dom; ip=$(get_ip); dom=$(get_domain)
    _tg_send "✅ <b>Akun Trojan Baru — MAX PANEL</b>
━━━━━━━━━━━━━━━━━━━━━━
👤 Remark   : <code>${u}</code>
🔑 Password : <code>${p}</code>
🌐 Host     : <code>${dom}</code>
...
🔗 <b>WS TLS (2083):</b>
<code>trojan://${p}@${dom}:2083?path=/trojan-ws&security=tls&host=${dom}&type=ws&sni=${dom}#${u}-WS-2083</code>

🔗 <b>WS TLS (2087):</b>
<code>trojan://${p}@${dom}:2087?path=/trojan-ws&security=tls&host=${dom}&type=ws&sni=${dom}#${u}-WS-2087</code>

🔗 <b>WS TLS (8443):</b>
<code>trojan://${p}@${dom}:8443?path=/trojan-ws&security=tls&host=${dom}&type=ws&sni=${dom}#${u}-WS-8443</code>

🔗 <b>gRPC TLS (443):</b>
<code>trojan://${p}@${dom}:443?mode=gun&security=tls&type=grpc&serviceName=trojan-grpc&sni=${dom}#${u}-gRPC</code>

🔗 <b>gRPC TLS (8443):</b>
<code>trojan://${p}@${dom}:8443?mode=gun&security=tls&type=grpc&serviceName=trojan-grpc&sni=${dom}#${u}-gRPC-ALT</code>"
    pause
}
...
ovpn_add() {
    ...
    echo -e "  ${A1}└─────────────────────────────────────────────────────────${NC}"
    echo ""
    _tg_send "✅ <b>Akun OpenVPN Baru — MAX PANEL</b>
━━━━━━━━━━━━━━━━━━━━━━
👤 Username : <code>${u}</code>
🔑 Password : <code>${p}</code>
━━━━━━━━━━━━━━━━━━━━━━
...
📅 Expired  : <code>${exp}</code>
━━━━━━━━━━━━━━━━━━━━━━
📎 Config dikirim di bawah ini 👇"
    _tg_send_file "/etc/openvpn/client/${u}/${u}-tcp.ovpn" "📄 <b>${u}</b> — OpenVPN TCP Config"
    _tg_send_file "/etc/openvpn/client/${u}/${u}-udp.ovpn" "📄 <b>${u}</b> — OpenVPN UDP Config"
    pause
}
...
do_backup() {
    ...
    if tar -czPf "$out" "${files[@]}" 2>/dev/null; then
        ...
        _tg_send "💾 <b>Backup Berhasil — MAX PANEL</b>
━━━━━━━━━━━━━━━━━━━━━━
📁 File : <code>$(basename "$out")</code>
📦 Size : <code>${sz}</code>
📅 Waktu: <code>$(date '+%d/%m/%Y %H:%M:%S')</code>
━━━━━━━━━━━━━━━━━━━━━━
📎 File backup dikirim di bawah ini 👇"
        _tg_send_file "$out" "💾 <b>MAX PANEL Backup</b> — $(basename "$out") (${sz})"
    else
        err "Backup gagal!"
    fi
    pause
...
do_restore() {
    ...
    echo -e "  ${A2}[1]${NC} Restore dari file lokal"
    echo -e "  ${A2}[2]${NC} Restore dari Telegram (download file terbaru)"
    echo ""
    echo -ne "  ${A3}Pilih${NC}: "; read -r restore_mode

    if [[ "$restore_mode" == "2" ]]; then
        # Restore dari Telegram
        [[ ! -f "$BOTF" ]] && { err "Bot Telegram belum dikonfigurasi!"; pause; return; }
        source "$BOTF" 2>/dev/null
        [[ -z "${BOT_TOKEN:-}" || -z "${CHAT_ID:-}" ]] && { err "BOT_TOKEN / CHAT_ID kosong!"; pause; return; }

        inf "Mengambil file backup terbaru dari Telegram..."
        # Ambil update terbaru dari bot
        local updates file_id file_name tmp_dl
        updates=$(curl -s "https://api.telegram.org/bot${BOT_TOKEN}/getUpdates?limit=20&allowed_updates=[\"message\"]")
        # Cari file_id dengan nama max-backup-*.tar.gz terbaru
        file_id=$(echo "$updates" | python3 -c "
import sys,json
data=json.load(sys.stdin)
results=data.get('result',[])
for r in reversed(results):
    doc=r.get('message',{}).get('document',{})
    name=doc.get('file_name','')
    if name.startswith('max-backup') and name.endswith('.tar.gz'):
        print(doc.get('file_id',''))
        break
" 2>/dev/null)
        file_name=$(echo "$updates" | python3 -c "
import sys,json
data=json.load(sys.stdin)
results=data.get('result',[])
for r in reversed(results):
...
" 2>/dev/null)
        ...
        if [[ -z "$file_id" ]]; then
            err "Tidak ada file backup ditemukan di Telegram!"
            warn "Pastikan file backup sudah pernah dikirim ke bot"
            pause; return
        fi

        inf "File ditemukan: ${W}${file_name}${NC}"
        inf "Mengunduh dari Telegram..."
        local file_path
        file_path=$(curl -s "https://api.telegram.org/bot${BOT_TOKEN}/getFile?file_id=${file_id}" | python3 -c "import sys,json; print(json.load(sys.stdin)['result']['file_path'])" 2>/dev/null)
        tmp_dl="$BACKUPDIR/${file_name}"
        mkdir -p "$BACKUPDIR"
        curl -s "https://api.telegram.org/file/bot${BOT_TOKEN}/${file_path}" -o "$tmp_dl"

        if [[ ! -f "$tmp_dl" || ! -s "$tmp_dl" ]]; then
            err "Gagal mengunduh file backup!"; pause; return
        fi
        ok "Download selesai: ${W}$(du -sh "$tmp_dl" | cut -f1)${NC}"
        ...
        inf "Restoring dari ${W}${file_name}${NC}..."
        tar -xzPf "$tmp_dl" -C / && ok "Restore selesai!" || err "Restore gagal!"
        systemctl daemon-reload
...
menu_cek_proxy() {
    show_header
    _top; _btn "  ${IT}${AL}🔍  CEK STATUS PROXY${NC}"; _bot; echo ""

    local ip dom; ip=$(get_ip); dom=$(get_domain)

    echo -e "  ${A1}${_DASH}${NC}"
    echo -e "  ${BLD}${AL}  🖧  ALL SSH / STUNNEL${NC}"
    echo -e "  ${A1}${_DASH}${NC}"
    printf "  ${DIM}%-20s %-10s %-6s %s${NC}\n" "Username" "Expired" "ML" "Status"
    echo -e "  ${A1}${_DASH}${NC}"
    while IFS='|' read -r u p exp ml _; do
        [[ -z "$u" ]] && continue
        local status today
        today=$(date +%Y-%m-%d)
        if [[ "$exp" < "$today" ]]; then
            status="${LR}EXPIRED${NC}"
        else
            # Cek apakah user sedang login
            local sess; sess=$(who | grep -c "^${u} " 2>/dev/null || echo 0)
            if [[ "$sess" -gt 0 ]]; then
                status="${LG}ONLINE(${sess})${NC}"
            else
                status="${A2}AKTIF${NC}"
            fi
        fi
        printf "  %-20s %-10s %-6s " "$u" "$exp" "$ml"
        echo -e "$status"
    done < <(cat "$SSH_DB" 2>/dev/null)

    echo ""
    echo -e "  ${A1}${_DASH}${NC}"
    echo -e "  ${BLD}${AL}  🟣  ALL V2RAY (VMess / VLess / Trojan)${NC}"
    echo -e "  ${A1}${_DASH}${NC}"
    printf "  ${DIM}%-20s %-10s %-10s %-6s${NC}\n" "Username" "Protokol" "Expired" "ML"
    echo -e "  ${A1}${_DASH}${NC}"

    while IFS='|' read -r u uuid _ exp ml _; do
        [[ -z "$u" ]] && continue
        local today; today=$(date +%Y-%m-%d)
        local status
        [[ "$exp" < "$today" ]] && status="${LR}EXPIRED${NC}" || status="${LG}AKTIF${NC}"
        printf "  %-20s %-10s %-10s %-6s " "$u" "VMess" "$exp" "$ml"
        echo -e "$status"
    done < <(cat "$VMESS_DB" 2>/dev/null)

    while IFS='|' read -r u uuid exp ml _; do
        [[ -z "$u" ]] && continue
        local today; today=$(date +%Y-%m-%d)
        local status
        [[ "$exp" < "$today" ]] && status="${LR}EXPIRED${NC}" || status="${LG}AKTIF${NC}"
        printf "  %-20s %-10s %-10s %-6s " "$u" "VLess" "$exp" "$ml"
        echo -e "$status"
    done < <(cat "$VLESS_DB" 2>/dev/null)
    ...
    while IFS='|' read -r u p exp ml _; do
        ...
        local status
        [[ "$exp" < "$today" ]] && status="${LR}EXPIRED${NC}" || status="${LG}AKTIF${NC}"
        printf "  %-20s %-10s %-10s %-6s " "$u" "Trojan" "$exp" "$ml"
        echo -e "$status"
    done < <(cat "$TROJAN_DB" 2>/dev/null)

    echo ""
    echo -e "  ${A1}${_DASH}${NC}"
    echo -e "  ${BLD}${AL}  ⚡  ALL TROJAN-GO / HYSTERIA / WireGuard${NC}"
    echo -e "  ${A1}${_DASH}${NC}"
    printf "  ${DIM}%-20s %-10s %-10s %-6s${NC}\n" "Username" "Protokol" "Expired" "ML"
    echo -e "  ${A1}${_DASH}${NC}"
    ...
    while IFS='|' read -r u p exp ml _; do
        ...
        local status
        [[ "$exp" < "$today" ]] && status="${LR}EXPIRED${NC}" || status="${LG}AKTIF${NC}"
        printf "  %-20s %-10s %-10s %-6s " "$u" "TrojanGo" "$exp" "$ml"
        echo -e "$status"
    done < <(cat "$TROJANGO_DB" 2>/dev/null)
    ...
    while IFS='|' read -r u p exp ml _; do
        ...
        local status
        [[ "$exp" < "$today" ]] && status="${LR}EXPIRED${NC}" || status="${LG}AKTIF${NC}"
        printf "  %-20s %-10s %-10s %-6s " "$u" "Hysteria" "$exp" "$ml"
        echo -e "$status"
    done < <(cat "$HY_DB" 2>/dev/null)
    ...
    while true; do
        ...
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${A2}" "[15] 🔍  Cek Semua Proxy    " "${LR}" "[E]  🗑  Uninstall"
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${LR}" "[X]  ✗   Keluar            " "${DIM}" ""
        echo -e "  ${A1}${_DASH}${NC}"
