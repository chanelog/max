...
# ════════════════════════════════════════════════════════════
main_menu() {
    # Cetak baris menu 2 kolom dengan lebar kolom kiri tetap 28 karakter visual
    _r2() {
        local CL="$1" TL="$2" CR="$3" TR="$4"
        # Kolom kiri: warna + teks (pad ke 28 char), lalu separator, lalu kolom kanan
        printf "  \e[0m${CL}%-28s\e[0m ${A1}│${NC}  ${CR}%s${NC}\n" "$TL" "$TR"
    }
