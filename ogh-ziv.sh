...
# Ambil daftar IP aktif per user dari log zivpn
# Format log: ... user=USERNAME ... addr=IP:PORT ...
get_user_ips() {
    local u="$1"
    # Coba parse dari log zivpn (format: user=X addr=IP:port)
    if [[ -f "$LOG" ]]; then
        grep "user=$u" "$LOG" 2>/dev/null \
...
monitor_maxlogin_ip() {
    ...
    while IFS='|' read -r uname maxdev; do
        ...
        local ip_count; ip_count=$(echo "$user_ips" | grep -c '[0-9]' || echo 0)
...
kick_violators() {
    ...
    while IFS='|' read -r uname maxdev; do
        ...
        local ip_count; ip_count=$(count_user_ips "$uname")
        if [[ "$ip_count" -gt "$maxdev" ]]; then
