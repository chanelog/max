...
load_theme() {
    ...
    case "$theme" in
        ...
        # ── TEMA BARU ──────────────────────────────────────────
        8) A1='\033[38;5;39m'; A2='\033[38;5;45m'; A3='\033[38;5;33m'; A4='\033[38;5;117m'
           AL='\033[38;5;51m'; AT='\033[1;37m'; THEME_NAME="OCEAN" ;;
        9) A1='\033[38;5;208m'; A2='\033[38;5;214m'; A3='\033[38;5;202m'; A4='\033[38;5;220m'
           AL='\033[38;5;215m'; AT='\033[1;37m'; THEME_NAME="SUNSET" ;;
       10) A1='\033[38;5;255m'; A2='\033[38;5;252m'; A3='\033[38;5;245m'; A4='\033[38;5;250m'
           AL='\033[38;5;231m'; AT='\033[1;37m'; THEME_NAME="CARBON" ;;
       11) A1='\033[38;5;157m'; A2='\033[38;5;120m'; A3='\033[38;5;85m'; A4='\033[38;5;192m'
           AL='\033[38;5;156m'; AT='\033[1;37m'; THEME_NAME="MATRIX" ;;
       12) A1='\033[38;5;183m'; A2='\033[38;5;147m'; A3='\033[38;5;189m'; A4='\033[38;5;219m'
           AL='\033[38;5;177m'; AT='\033[1;37m'; THEME_NAME="LAVENDER" ;;
...
menu_tema() {
    while true; do
        ...
        local themes=(
            ...
            "4"  "💛" "GOLD"     "Emas Mewah"
            "5"  "❤️" "RED"      "Merah Elegan"
            "6"  "🩷" "PINK"     "Pink Pastel"
            "7"  "🌈" "RAINBOW"  "Pelangi Cantik"
            "8"  "🌊" "OCEAN"    "Biru Laut Elegan"
            "9"  "🌅" "SUNSET"   "Oranye Senja Kekinian"
            "10" "🖤" "CARBON"   "Abu Gelap Minimalis"
            "11" "💻" "MATRIX"   "Hijau Hacker Keren"
            "12" "🪻" "LAVENDER" "Ungu Muda Aesthetic"
        )

        local i=0
        while [[ $i -lt ${#themes[@]} ]]; do
            local num="${themes[$i]}"
            local ico="${themes[$((i+1))]}"
            local name="${themes[$((i+2))]}"
            local desc="${themes[$((i+3))]}"
            local mark="   "
            [[ "$cur_theme" == "$num" ]] && mark="${A2}▶${NC} "
            if [[ "$num" == "7" ]]; then
                printf "    %b${ico}  ${A1}[%-2s]${NC}  \033[38;5;196mR\033[38;5;208mA\033[38;5;226mI\033[38;5;82mN\033[38;5;51mB\033[38;5;171mO\033[38;5;213mW\033[0m  — %s\n" "$mark" "$num" "$desc"
            else
                printf "    %b${ico}  ${A1}[%-2s]${NC}  %-10s — %s\n" "$mark" "$num" "$name" "$desc"
            fi
            i=$((i+4))
        done
        ...
        echo -e "  ${A1}${_DASH}${NC}"
        echo ""
        echo -ne "  ${A1}›${NC} Pilih tema [0-12]: "; read -r ch
        case $ch in
            [1-9]|10|11|12)
...
SVEOF
    ...
    inf "Memasang cron jobs otomatis..."
    setup_cron_jobs
    ok "Cron jobs terpasang (cek service, expired, auto backup)"
...
# ════════════════════════════════════════════════════════════
#  FITUR TAMBAHAN — CRON, MONITOR, NOTIF CRASH, AUTO BACKUP
# ════════════════════════════════════════════════════════════

AUTOCF="$DIR/autocron.conf"   # format: AUTOBACKUP=1|0  JAM=00

# ── 1. CRON EXPIRED AKUN OTOMATIS ───────────────────────────
# Dipanggil oleh cron setiap hari → hapus akun expired otomatis
auto_clean_expired() {
    [[ ! -f "$UDB" ]] && return
    local today; today=$(date +"%Y-%m-%d")
    local cnt=0 deleted_list=""
    while IFS='|' read -r u _ e _ _; do
        if [[ -n "$e" && "$e" < "$today" ]]; then
            sed -i "/^${u}|/d" "$UDB" 2>/dev/null
            del_maxlogin "$u"
            ((cnt++))
            deleted_list+="• $u (exp: $e)\n"
        fi
    done < <(cat "$UDB" 2>/dev/null)
    if [[ $cnt -gt 0 ]]; then
        _reload_pw
        _tg_send "🧹 <b>Auto-Clean Expired</b>
⏰ $(date '+%d/%m/%Y %H:%M')
🗑 <b>${cnt} akun</b> expired dihapus otomatis:
$(echo -e "$deleted_list")"
    fi
}

# ── 2. NOTIFIKASI TELEGRAM SAAT SERVICE CRASH ───────────────
# Dipanggil oleh cron setiap menit → cek status zivpn
auto_check_service() {
    local FLAGF="/tmp/oghziv_svc_down.flag"
    if ! systemctl is-active --quiet zivpn 2>/dev/null; then
        # Coba restart otomatis
        systemctl restart zivpn &>/dev/null
        sleep 3
        if systemctl is-active --quiet zivpn 2>/dev/null; then
            # Berhasil restart — kirim notif recover (hanya jika sebelumnya down)
            if [[ -f "$FLAGF" ]]; then
                rm -f "$FLAGF"
                _tg_send "✅ <b>ZiVPN Pulih!</b>
🔄 Service berhasil di-restart otomatis
🕐 $(date '+%d/%m/%Y %H:%M')
🖥 IP: $(get_ip)"
            fi
        else
            # Masih mati — kirim notif jika belum ada flag
            if [[ ! -f "$FLAGF" ]]; then
                touch "$FLAGF"
                _tg_send "🚨 <b>ZiVPN SERVICE DOWN!</b>
⚠️ Service mati & gagal restart otomatis!
🕐 $(date '+%d/%m/%Y %H:%M')
🖥 IP: $(get_ip)
📋 Cek log: journalctl -u zivpn -n 20"
            fi
        fi
    else
        # Service normal, hapus flag jika ada
        [[ -f "$FLAGF" ]] && rm -f "$FLAGF"
    fi
}
...
auto_backup_run() {
    ...
    _bak_collect
    [[ ${#BAK_FILES[@]} -eq 0 ]] && return

    tar -czPf "$bfile" "${BAK_FILES[@]}" 2>/dev/null
    if [[ -s "$bfile" ]]; then
        _bak_cleanup_old 10
        _bak_tg_send_file "$bfile" &>/dev/null
    fi
}

# ── 4. SETUP SEMUA CRON JOBS ────────────────────────────────
setup_cron_jobs() {
    local script_path="/usr/local/bin/ogh-ziv"
    # Hapus cron ogh-ziv lama dulu
    crontab -l 2>/dev/null | grep -v "ogh-ziv" | crontab - 2>/dev/null

    # Baca jam backup dari config
    local bak_jam="0"
    [[ -f "$AUTOCF" ]] && { source "$AUTOCF" 2>/dev/null; bak_jam="${JAM:-0}"; }

    # Tambah cron jobs baru
    (
        crontab -l 2>/dev/null
        echo "# OGH-ZIV — cek service crash setiap menit"
        echo "* * * * * bash $script_path --check-service >> /tmp/oghziv_cron.log 2>&1"
        echo "# OGH-ZIV — hapus akun expired setiap hari jam 00:05"
        echo "5 0 * * * bash $script_path --clean-expired >> /tmp/oghziv_cron.log 2>&1"
        echo "# OGH-ZIV — auto backup harian"
        echo "0 ${bak_jam} * * * bash $script_path --auto-backup >> /tmp/oghziv_cron.log 2>&1"
        echo "# OGH-ZIV — cek maxlogin setiap 5 menit"
        echo "*/5 * * * * bash $script_path --check-maxlogin >> /tmp/oghziv_cron.log 2>&1"
    ) | crontab - 2>/dev/null
}

# ── 5. MONITOR USER ONLINE REALTIME ─────────────────────────
svc_monitor_online() {
    show_header
    _top; _btn "  ${IT}${AL}📡  MONITOR USER ONLINE REALTIME${NC}"; _bot; echo ""
    local port; port=$(get_port)
    local today; today=$(date +%Y-%m-%d)

    echo -e "  ${DIM}Port ZiVPN :${NC} ${Y}$port${NC}"
    echo -e "  ${DIM}Waktu      :${NC} ${W}$(date '+%d/%m/%Y %H:%M:%S')${NC}"
    echo ""
    echo -e "  ${A1}${_DASH}${NC}"
    printf  "  ${BLD}%-20s  %-22s  %-10s  %-8s${NC}\n" "Username" "Alamat IP" "Expired" "Status"
    echo -e "  ${A1}${_DASH}${NC}"

    # Ambil koneksi UDP aktif ke port zivpn
    local conn_raw; conn_raw=$(ss -u -n -p 2>/dev/null | grep ":$port")
    local total_online=0

    if [[ -f "$UDB" ]]; then
        while IFS='|' read -r u p e q _; do
            # Cek apakah user ada di koneksi aktif (via log)
            local is_online="${LR}Offline${NC}"
            local conn_ip="-"
            if [[ -f "$LOG" ]] && grep -q "user=$u" "$LOG" 2>/dev/null; then
                local last_seen; last_seen=$(grep "user=$u" "$LOG" 2>/dev/null | tail -1 | cut -c1-16)
                is_online="${LG}Online${NC}"
                conn_ip="${last_seen:-?}"
                ((total_online++))
            fi
            local exp_col="${LG}"
            [[ "$e" < "$today" ]] && exp_col="${LR}"
            printf "  ${W}%-20s${NC}  ${DIM}%-22s${NC}  ${exp_col}%-10s${NC}  %b\n" \
                "$u" "$conn_ip" "$e" "$is_online"
        done < "$UDB"
    fi

    echo -e "  ${A1}${_DASH}${NC}"
    echo ""
    echo -e "  ${DIM}Koneksi UDP aktif ke port $port :${NC}"
    echo ""
    if [[ -n "$conn_raw" ]]; then
        echo "$conn_raw" | awk '{printf "  %-45s %s\n", $5, $6}' | head -20
    else
        inf "Tidak ada koneksi UDP aktif saat ini."
    fi
    echo ""
    echo -e "  ${A1}${_DASH}${NC}"
    printf  "  ${DIM}Total akun terdaftar:${NC} ${W}$(total_user)${NC}  ${A1}│${NC}  ${DIM}Estimasi online:${NC} ${LG}${total_online}${NC}"
    echo ""
    echo -e "  ${A1}${_DASH}${NC}"
    pause
}

# ── 6. MENU PENGATURAN AUTO BACKUP ──────────────────────────
menu_autobackup() {
    while true; do
        show_header
        [[ -f "$AUTOCF" ]] && source "$AUTOCF" 2>/dev/null
        local ab_status ab_col ab_jam
        if [[ "$AUTOBACKUP" == "1" ]]; then
            ab_status="AKTIF ✅"; ab_col="${LG}"
        else
            ab_status="NONAKTIF ❌"; ab_col="${LR}"
        fi
        ab_jam="${JAM:-0}"

        _top
        _btn "  ${IT}${AL}  ⏰  AUTO BACKUP HARIAN${NC}"
        _sep
        _btn "  ${DIM}Status :${NC}  ${ab_col}${ab_status}${NC}"
        _btn "  ${DIM}Jam    :${NC}  ${Y}${ab_jam}:00${NC} setiap hari"
        _sep
        _btn "  ${A2}[1]${NC}  ✅  Aktifkan Auto Backup"
        _sep
        _btn "  ${A2}[2]${NC}  ❌  Nonaktifkan Auto Backup"
        _sep
        _btn "  ${A2}[3]${NC}  🕐  Ganti Jam Backup"
        _sep
        ...
        case $ch in
        1)
            echo -ne "  ${A3}Jam backup (0-23)${NC} [${ab_jam}]: "; read -r jm
            [[ ! "$jm" =~ ^[0-9]+$ || $jm -lt 0 || $jm -gt 23 ]] && jm="${ab_jam}"
            printf "AUTOBACKUP=1\nJAM=%s\n" "$jm" > "$AUTOCF"
            setup_cron_jobs
            ok "Auto Backup DIAKTIFKAN — setiap hari jam ${jm}:00"
            sleep 1 ;;
        2)
            printf "AUTOBACKUP=0\nJAM=%s\n" "$ab_jam" > "$AUTOCF"
            setup_cron_jobs
            ok "Auto Backup DINONAKTIFKAN"
            sleep 1 ;;
        3)
            echo -ne "  ${A3}Jam backup baru (0-23)${NC}: "; read -r jm2
            [[ ! "$jm2" =~ ^[0-9]+$ || $jm2 -lt 0 || $jm2 -gt 23 ]] && {
                err "Jam tidak valid!"; sleep 1; continue; }
            printf "AUTOBACKUP=%s\nJAM=%s\n" "${AUTOBACKUP:-0}" "$jm2" > "$AUTOCF"
            setup_cron_jobs
            ok "Jam backup diubah ke ${jm2}:00"
            sleep 1 ;;
        0) break ;;
...
menu_service() {
    while true; do
        ...
        _sep; _btn "  ${A2}[A]${NC}  📡  Monitor User Online"
        _sep; _btn "  ${A2}[B]${NC}  ⏰  Auto Backup Harian"
        _sep; _btn "  ${LR}[0]${NC}  ◀   Kembali"
        _bot; echo ""
        echo -ne "  ${A1}›${NC} "; read -r ch
        case ${ch,,} in
